...
use num::{one, Num, CheckedSub, CheckedAdd};
use std::fmt::{Debug, Display};

pub trait Instruction: Clone + Copy + Debug {}
impl<T> Instruction for T where T: Clone + Debug + Copy {}

pub trait Value: Clone + Display + Default {}
impl<T> Value for T where T: Clone + Display + Default {}

pub trait Index: Copy + Default {
    /// Increment (normally by one).
    fn incr(self) -> Self;
    /// Decrement (normally by one).
    fn decr(self) -> Self;
}

impl<N: CheckedSub + CheckedAdd + Num + Copy + Default> Index for N {
    fn incr(self) -> Self {
        self.checked_add(&one()).expect("Index overflow!")
    }
    fn decr(self) -> Self {
        self.checked_sub(&one()).expect("Index overflow!")
    }
}